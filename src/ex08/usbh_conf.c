...
USBH_StatusTypeDef USBH_LL_Init(USBH_HandleTypeDef *phost) {
	...
	usbHCD.Instance = USB_OTG_FS;
	usbHCD.Init.Host_channels = 11;
	usbHCD.Init.dma_enable = 0;
	usbHCD.Init.low_power_enable = 0;
	usbHCD.Init.phy_itface = HCD_PHY_EMBEDDED;
	usbHCD.Init.Sof_enable = 0;
	usbHCD.Init.speed = HCD_SPEED_FULL;
	usbHCD.Init.vbus_sensing_enable = 0;
	...
	USBH_LL_SetTimer(phost, HAL_HCD_GetCurrentFrame(&usbHCD));
...
USBH_StatusTypeDef USBH_LL_SetToggle(USBH_HandleTypeDef *phost, uint8_t pipe,
		uint8_t toggle) {
	if (usbHCD.hc[pipe].ep_is_in) {
		usbHCD.hc[pipe].toggle_in = toggle;
	} else {
		usbHCD.hc[pipe].toggle_out = toggle;
	}
	return USBH_OK;
}
...
uint8_t USBH_LL_GetToggle(USBH_HandleTypeDef *phost, uint8_t pipe) {
	...
	if (usbHCD.hc[pipe].ep_is_in) {
		toggle = usbHCD.hc[pipe].toggle_in;
	} else {
		toggle = usbHCD.hc[pipe].toggle_out;
	}
	return toggle;
}
